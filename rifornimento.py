...
def esegui_rifornimento(porta: str, nome: str,
    ...
    # Risorse da considerare (quelle con qta > 0 in config), in ordine di priorità
    risorse_config = {r: q for r, q in quantita.items() if q > 0}
    if not risorse_config:
        log("Nessuna risorsa configurata - skip")
        return 0

    log(f"Risorse configurate: {list(risorse_config.keys())} | soglia: {soglia}M")
    ...
    while True:
        ...
        # 3. Seleziona risorsa da inviare (rotazione, verifica soglia)
        risorsa_scelta = None
        for i in range(len(risorse_lista)):
            r = risorse_lista[(idx_risorsa + i) % len(risorse_lista)]
            valore_r = risorse_reali.get(r, -1)
            qta_r    = risorse_config[r]
            necessario = qta_r * (1 + TASSA_DEFAULT)  # stima conservativa pre-maschera
            soglia_abs = soglia * 1e6  # soglia in unità assolute
            log(f"  Check {r}: valore={valore_r/1e6:.1f}M soglia={soglia_abs/1e6:.1f}M → {'OK' if valore_r >= soglia_abs else 'SOTTO'}")
